...
async function fetchWithRetry(url: string, options: any, maxRetries = 10) {
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      if (
        text.includes("Too many backlogged requests") ||
        text.includes("try again later?")
      ) {
        const backoff = Math.pow(2, attempt) * 1000;
        console.warn(
          `[RETRY] Grist backlogged. Attempt ${
            attempt + 1
...
Bun.serve({
  ...
  async fetch(req) {
    ...
    if (
      pathParts.length === 4 &&
      (pathParts[1] === "download" || pathParts[1] === "metadata")
    ) {
      ...
      try {
        ...
        if (!response.ok)
          return new Response(`Grist Error: ${response.statusText}`, {
        ...
        async start(controller) {
          const send = (data: any) => {
            ...
            try {
              ...
              console.error(
                "[DEBUG] Failed to enqueue data, controller likely closed",
                e
              );
              isClosed = true;
            }
          ...
          // Unique per run so concurrent runs for the same batch don't clobber each other
          const tmpDir = path.join(
            "/tmp",
            `${platesOrderId}-${crypto.randomUUID()}`
          );
          const zipPath = `${tmpDir}.zip`;

          try {
            console.log(
              `[DEBUG] Starting workflow for batch: ${platesOrderId}`
            );

            // 1. Query Plates_Order_Batches to get internal ID
            console.log(
              `[DEBUG] Querying Plates_Order_Batches for ID: ${platesOrderId}...`
            );
            send({ stage: "Locating Batch Record", status: "started" });

            const batchFilter = JSON.stringify({
              Plates_Order_ID: [platesOrderId],
            });
            const batchUrl = `${GRIST_SERVER}/api/docs/${GRIST_DOC_ID}/tables/Plates_Order_Batches/records?filter=${encodeURIComponent(
              batchFilter
            )}`;
            ...
            if (!batchResponse.ok) {
              throw new Error(
                `Grist Batch Query Error: ${batchResponse.statusText}`
              );
            }
            ...
            if (existingLink && existingLink.trim() !== "") {
              console.log(
                `[DEBUG] Existing download link found: ${existingLink}. Skipping processing.`
              );
              send({
                stage: "Workflow Completed",
                status: "success",
                url: existingLink,
                message: "Returned existing link from Grist.",
              });
              return; // Exit early
            }
            ...
            console.log(
              `[DEBUG] Querying Plate_Orders for batch ID: ${internalBatchId}...`
            );
            send({ stage: "Searching Plate Order Records", status: "started" });
            const plateFilter = JSON.stringify({
              Plate_Orders_Batch: [internalBatchId],
            });
            const plateUrl = `${GRIST_SERVER}/api/docs/${GRIST_DOC_ID}/tables/Plate_Orders/records?filter=${encodeURIComponent(
              plateFilter
            )}`;
            ...
            if (!gristResponse.ok) {
              throw new Error(
                `Grist Plate Query Error: ${gristResponse.statusText}`
              );
            }
            ...
            send({
              stage: "Searching Plate Order Records",
              status: "completed",
              count: plates.length,
            });
            ...
            console.log(
              `[DEBUG] Starting downloads for ${plates.length} plates (limit: 50 at a time)...`
            );
            send({
            ...
            const workers = Array(Math.min(CONCURRENCY_LIMIT, plates.length))
              .fill(null)
              .map(async () => {
                while (downloadQueue.length > 0) {
                  ...
                  if (!resp.ok) {
                    console.log(resp);
                    throw new Error(
                      `Failed to download attachment ${plate.attId}`
                    );
                  }
                  ...
                  send({
                    ...
                    status: "progress",
                    progress: Math.round(
                      (downloadedCount / plates.length) * 100
                    ),
                    current: filename,
                  });
                }
            ...
            // 4. S3 Upload
            console.log(
              `[DEBUG] Starting S3 upload to bucket: ${S3_BUCKET}...`
            );
            ...
            console.log(
              `[DEBUG] Updating Grist record ${internalBatchId} with download link...`
            );
            ...
            if (!updateResponse.ok) {
              throw new Error(
                `Failed to update Grist with download link: ${updateResponse.statusText}`
              );
            }
            ...
            console.log(
              `[DEBUG] Workflow finished successfully for ${platesOrderId}`
            );
          } catch (error: any) {
            console.error(error);
            send({ stage: "Error", status: "failed", message: error.message });
          } finally {
            // Cleanup ephemeral storage, including after failures
